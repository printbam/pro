...
async function initGlobalRealtimeListeners() {
    ...
    console.log("🚀 Initialisation du service global Realtime (Pro)...");
    ...
    // 3. ÉCOUTEUR A : NOUVEAUX MESSAGES (CHAT)
    globalMessageListener = supabaseClient
        ...
        .on('postgres_changes', {
            ...
            // B. Vérifier si ce chat nous appartient
            if (!myChatIdsCache.has(msg.chat_id)) {
                ...
                // C'est un nouveau chat, on l'ajoute au cache
                myChatIdsCache.add(msg.chat_id);
            }

            // C. Si on est DÉJÀ dans ce chat -> On ne fait rien (géré par le listener local)
            if (window.currentChatId === msg.chat_id) {
                return;
            }

            // D. Récupérer les infos de l'expéditeur
            ...
            // E. Afficher le Toast Message Style
            if (senderProfile) {
    ...
    // 4. ÉCOUTEUR B : NOTIFICATIONS SOCIALES (LIKES, COMMENTS, FOLLOWS)
    globalNotifListener = supabaseClient
        ...
        .on('postgres_changes', {
            ...
            schema: 'public',
            table: 'notifications',
            filter: `recipient_id=eq.${user.id}` // Important : que mes notifs
        }, async (payload) => {
            const notif = payload.new;

            // A. Récupérer le profil de l'acteur (Celui qui fait l'action)
            // Realtime ne renvoie que l'ID, on doit chercher le nom/photo
            ...
            // B. Afficher le Toast Pro (Défini précédemment)
            if (actorProfile) {
                showProNotificationToast({
                    ...notif, // Contient type, post_id, content, created_at
                    actor: actorProfile // On attache les infos du profil
                });
            } else {
                // Fallback si le profil n'est pas trouvé
        ...
        .subscribe((status) => {
            if (status === 'SUBSCRIBED') console.log("✅ Écoute notifications active");
        });
}
...
async function loadNotifications() {
    ...
    // Loader
    container.innerHTML = `<div class="text-center py-10 text-slate-400"><i class="fas fa-spinner fa-spin text-2xl"></i></div>`;

    try {
        // 1. Récupérer les notifications avec les infos de l'acteur
        const { data: notifs, error } = await supabaseClient
            .from('notifications')
            .select('*, actor:profiles!notifications_actor_id_fkey(id, full_name, avatar_url, is_certified)')
        ...
        if (!notifs || notifs.length === 0) {
            container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center">
        ...
        container.innerHTML = notifs.map(notif => {
            ...
            const avatar = actor.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(actor.full_name || 'User')}`;
            const time = typeof getTimeAgo === 'function' ? getTimeAgo(new Date(notif.created_at)) : '';
            const isRead = notif.is_read;

            // Contenu dynamique
            let icon = '<i class="fas fa-bell text-slate-400"></i>';
            let text = 'Interaction inconnue';
            let clickAction = '';

            if (notif.type === 'like') {
                icon = '<i class="fas fa-heart text-red-500"></i>';
                text = 'a aimé votre publication.';
                clickAction = `openPostModal('${notif.post_id}')`;
            } else if (notif.type === 'comment') {
                icon = '<i class="fas fa-comment text-blue-500"></i>';
                text = `a commenté : "${notif.content?.substring(0, 30) || '...'}"`;
                clickAction = `openPostModal('${notif.post_id}')`;
            } else if (notif.type === 'follow') {
                icon = '<i class="fas fa-user-plus text-indigo-500"></i>';
                text = 'a commencé à vous suivre.';
                clickAction = `viewUserProfile('${actor.id}')`;
            }

            // Style "Non lu" (Fond bleu très clair)
            const bgClass = isRead ? 'bg-white' : 'bg-indigo-50';

            return `
            <div class="flex items-center gap-3 p-4 border-b border-slate-100 dark:border-slate-800 cursor-pointer transition-all ${bgClass} hover:bg-slate-50 dark:hover:bg-slate-700"
                 onclick="${clickAction}">
                
                <div class="relative flex-shrink-0">
                    <img src="${avatar}" class="w-12 h-12 rounded-full object-cover shadow-sm">
                    <div class="absolute -bottom-1 -right-1 bg-white dark:bg-slate-800 rounded-full p-1 text-xs">
                        ${icon}
                    </div>
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm text-slate-800 dark:text-slate-200">
                        <span class="font-bold">${actor.full_name}</span> 
                        ${text}
                    </p>
...
async function initialiserApplication() {
    try {
        ...
        await Promise.allSettled([
            ...
            charger("Stories", loadStories, 100),
...
async function openStoryViewer(userId) {
    ...
    if (error || !stories || stories.length === 0) {
        showToast("Story non disponible", "error");
        return;
    }
...
async function handleStoryUpload(input) {
    ...
    try {
        ...
        const { data: uploadData, error: uploadError } = await supabaseClient.storage
            .from('stories') // Créez un bucket 'stories' dans Supabase
            .upload(fileName, file);
...
/**
 * Affiche une notification Toast style Facebook/Instagram
 * @param {Object} data - Données de la notification
 */
function showProNotificationToast(data) {
    const container = document.getElementById('toast-container');
    if (!container || !data.actor) return;
    ...
    const icons = {
        ...
        'comment': '<i class="fas fa-comment-dots text-blue-500"></i>',
    ...
    const messages = {
        ...
        'comment': 'a commenté : ' + (data.content?.substring(0, 20) || '...'),
        'follow': 'a commencé à vous suivre.',
        'share': 'a partagé votre publication.'
    };

    // Avatar (avec fallback)
    const avatar = data.actor.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.actor.full_name)}&background=random`;

    // HTML Interne du Toast
    toast.innerHTML = `
        <div class="toast-content-flex" onclick="handleToastClick('${data.type}', '${data.post_id}', '${data.actor.id}')">
            <div class="relative flex-shrink-0">
                <img src="${avatar}" class="w-11 h-11 rounded-full object-cover border-2 border-white shadow-sm">
                <div class="toast-icon-badge">
                    ${icons[data.type] || '<i class="fas fa-bell"></i>'}
                </div>
            </div>
            <div class="flex-1 pl-3 overflow-hidden">
                <div class="flex justify-between items-baseline">
                    <span class="font-bold text-slate-900 text-sm truncate">${data.actor.full_name}</span>
                    <span class="text-[10px] text-slate-400 ml-2">${getTimeAgo(new Date(data.created_at))}</span>
                </div>
                <p class="text-xs text-slate-600 truncate">
                    ${messages[data.type] || 'Nouvelle interaction'}
                </p>
            </div>
        </div>
        <button onclick="this.parentElement.remove()" class="absolute top-2 right-2 text-slate-300 hover:text-slate-600">
...
// Fonction utilitaire pour gérer le clic sur le Toast
function handleToastClick(type, postId, actorId) {
    if (type === 'follow') {
...
window.loadShortsView = async function() {
    ...
    try {
        ...
        const authorIds = [...new Set(shorts.map(p => p.user_id))];

        // 2. Récupérer mes Likes
        ...
        container.innerHTML = shorts.map((post) => {
            ...
            <div class="short-card" data-post-id="${post.id}">
                ...
                <div class="short-video-wrapper" onclick="toggleVideoPause(this)">
                    ...
                    <!-- BOUTONS SKIP (5s) -->
                    <button class="short-skip-btn left" onclick="event.stopPropagation(); skipVideoTime(-5, '${post.id}')">
                        <i class="fas fa-backward"></i>
                    </button>
                    <button class="short-skip-btn right" onclick="event.stopPropagation(); skipVideoTime(5, '${post.id}')">
                ...
                <div class="short-actions-left">
                    <!-- LIKE -->
                    <div class="short-action-btn ${isLiked ? 'liked' : ''}" id="top-like-btn-${post.id}" onclick="event.stopPropagation(); toggleShortLike('${post.id}', this)">
...
window.toggleVideoPause = function(wrapperElement) {
    ...
    if (video.paused) {
        // On tente de lire
        video.play().catch(e => console.log("Lecture interrompue")); // Sécurisé
        pauseIcon.innerHTML = '<i class="fas fa-play"></i>';
        wrapperElement.classList.remove('paused');
    } else {
        video.pause();
        pauseIcon.innerHTML = '<i class="fas fa-pause"></i>';
        wrapperElement.classList.add('paused');
    }
...
// 2. Barre de progression cliquable (Seek)
window.seekVideoProgress = function(event, postId) {
    ...
    // Calcul du pourcentage du clic
    const percent = (event.clientX - rect.left) / rect.width;
    
    // Mise à jour du temps vidéo
    video.currentTime = percent * video.duration;
}

// 3. Boutons Skip (5 secondes)
window.skipVideoTime = function(seconds, postId) {
    const video = document.querySelector(`[data-post-id="${postId}"] video`);
    if (video) {
